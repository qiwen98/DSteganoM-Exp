...
def train_model(train_loader, beta, learning_rate, test_loader, n_epochs_stop=20):
    ...
    for epoch in range(0, num_epochs):
        ...
        logger.info('{0},{1}, {2}, {3}, {4}'.format(epoch+1, num_epochs, mean_train_loss,mean_reveal_loss,mean_test_loss))
...
if __name__=='__main__':
    ...
    net.load_state_dict(torch.load('checkpoints\\HypStegNet\\Epoch N100.pkl', map_location=device))
    net.to(device)

    logging.basicConfig(filename='{}.log'.format(args.dataset), filemode='w', format='%(name)s - %(levelname)s - %(message)s')
    ...
    if dataset == "ESC50":
        ...
        train_loader = torch.utils.data.DataLoader(filter_train_dataset, batch_size=2, shuffle=True, num_workers=0,
                                drop_last=True)  # ,worker_init_fn=seed_worker,generator=g)
        test_loader = torch.utils.data.DataLoader(filter_val_dataset, batch_size=2, shuffle=True, num_workers=0,
