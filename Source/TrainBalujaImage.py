...
from TinyImage import TrainTinyImageNetDataset,TestTinyImageNetDataset
torch.manual_seed(0)
...
parser.add_argument("--dataset", type=str, default="Imagedataset", help="Dataset, default is MTM.")
...
CONFIG = {"dataset":"Balujadataset",
          "type":"variant-3",
          "epochs": 30,
          "learning-rate": 0.001,
          "module":"Baluja_image_{}Block_{}_layer_M_Dec".format(args_opt.module,args_opt.DEC_M_N_LAYERS)
          }
...
if __name__ == "__main__":
    ...
    train_loader = torch.utils.data.DataLoader(trainset, batch_size=2, shuffle=True, num_workers=2,drop_last=True)

    testset = TestTinyImageNetDataset(id=id_dict, transform=transform)
    val_loader = torch.utils.data.DataLoader(testset, batch_size=2, shuffle=False, num_workers=2,drop_last=True)
    test_loader = torch.utils.data.DataLoader(testset, batch_size=2, shuffle=False, num_workers=2,drop_last=True)
